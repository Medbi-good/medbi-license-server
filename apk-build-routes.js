...
async function runBuild(job, { owner, repoName, token, appName, packageId, mode, liveUrl, files, iconBase64, outputFormat, exitAppButton }) {
  try {
    ...
    if (exitAppButton) log(job, 'botão de sair da app ativado — vai ser adicionado nativamente ao ecrã.', 'dim');
      ...
      `/repos/${owner}/${repoName}/actions/workflows/build-apk.yml/dispatches`,
      {
        ...
        body: JSON.stringify({
          ...
          inputs: {
            ...
            exit_app_button: exitAppButton ? 'true' : 'false',
          },
        }),
      },
      token
...
//         exitAppButton?: boolean (default false) — adiciona um botão nativo "Sair"
...
router.post('/start', express.json({ limit: '100mb' }), (req, res) => {
  const { repo, appName, packageId, mode, liveUrl, files, iconBase64, outputFormat, exitAppButton } = req.body || {};
  const effectiveMode = mode === 'url' ? 'url' : 'file';
  const effectiveFormat = outputFormat === 'aab' ? 'aab' : 'apk';
  const effectiveExitAppButton = exitAppButton === true;
  ...
  runBuild(job, { owner, repoName, token: GITHUB_TOKEN, appName, packageId, mode: effectiveMode, liveUrl, files: files || [], iconBase64, outputFormat: effectiveFormat, exitAppButton: effectiveExitAppButton });
});
