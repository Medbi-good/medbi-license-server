...
// Data-limite fixa da promoção "primeiros 50 grátis": ninguém recebe acesso
// grátis para além desta data, seja qual for a data em que ativou.
// Formato: 'YYYY-MM-DDTHH:mm:ssZ' (UTC). Ajusta aqui se precisares de mudar a data.
...
// Promoción "primeros 50 gratis" — activa acceso gratis automáticamente,
// sin código, mientras queden cupos (apps.promo_limit / promo_used).
// NUEVO: el acceso gratis nunca dura más allá de PROMO_HARD_CUTOFF, sin importar
// cuándo se active el dispositivo (antes esto era siempre "hoy + N días").
// Después del cupo 50, o después de la fecha límite, el dispositivo debe
// activar con código pago normal.
app.post('/api/store/free-activate', async (req, res) => {
  ...
  try {
    ...
    // Si ya pasamos la fecha límite de la promo, no se emiten más licencias gratis,
    // sin importar cuántos cupos queden.
    if (Date.now() >= new Date(PROMO_HARD_CUTOFF).getTime()) {
    ...
    const license = generateLicense();
    // La expiración es lo que venga antes: hoy + duración normal, o la fecha límite fija.
    await pool.query(
      'INSERT INTO licenses (license, code, app_id, device_id, expires_at) VALUES ($1, $2, $3, $4, LEAST(now() + ($5 || \' days\')::interval, $6::timestamptz))',
      [license, promoCode, appId, deviceId, durationDays, PROMO_HARD_CUTOFF]
    );
...
app.get('/api/store/promo-status', async (req, res) => {
  ...
  try {
    ...
    const pastCutoff = Date.now() >= new Date(PROMO_HARD_CUTOFF).getTime();
    res.json({
      active: used < promo_limit && !pastCutoff,
